...
/**
 * An NotEqualCondition object represents one part of rule statement
 * of the form (prop != value) as defined by the rule language grammar.
 * Inequality between properties in an EvaluationContext and the value specified in
 * a rule is determined by a Java class that implements the Comparable interface
 * that is mapped to the syntax of the property being tested for inequality.
 * A Comparable guarantees that an apples to apples comparison is made.
 *
 * @author A. Rodriguez
 */
public class NotEqualCondition extends RuleCondition {

    /**
     * Constructs an NotEqualExpression to test if the property named by
     * <tt>name</tt> is not equal to the value <tt>value</tt>.
     */
    public NotEqualCondition(String name, String value) {
        super(name, value);
    }

    /**
     * Creates an empty NotEqualCondition.
     */
     ...
     * is not equal to the <tt>value</tt> in this expression.
     *
    ...
    /**
     * Returns the String representation of an NotEqualCondition.
     */
